...
#ifdef KTD_DEBUG
#define LOG_DBG(fmt, args...) pr_debug(KERN_INFO "[ktd]"fmt"\n", ##args)
#endif
...
void ktd3137_brightness_set_workfunc(struct ktd3137_chip *chip, int brightness)
{
	...
	if (brightness == 0) {
		...
		if (ktd_hbm_mode == 3) {
			...
			pr_debug(KERN_INFO "[%s]: ktd_hbm_mode = %d\n", __func__, ktd_hbm_mode);
...
int ktd3137_brightness_set(int brightness)
{
	...
	if ((brightness < 5) && (brightness > 2)) {//HQ-61731
		brightness = 5;
		pr_debug("%s: lyd_lowest_brightness, brightness = %d", __func__, brightness);
	}
#endif
